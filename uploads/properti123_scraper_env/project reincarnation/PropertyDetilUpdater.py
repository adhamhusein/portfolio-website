...
def run_batch_processing(batch_size=1000):
    ...
    while True:
        ...
        links = updater.db_connector.fetch_data(
            "SELECT url, status, available_data FROM public.property_link WHERE available_data = 0 LIMIT %s OFFSET %s",
            (batch_size, offset)
        )
        ...
        print(f"Processing batch starting at offset {offset} with size {len(links)}...")
        updater.update_property_details(links)
        offset += batch_size


if __name__ == "__main__":
    run_batch_processing(batch_size=1000)  # adjust the batch size as needed


# if __name__ == "__main__":
#     updater = PropertyDetailsUpdater()
#     link_updater = PropertyLinkUpdater()
#     links = link_updater.fetch_new_links(5)
#     updater.update_property_details(links)