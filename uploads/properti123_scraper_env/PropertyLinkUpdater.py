    ...
    def fetch_new_links(self, page_length):
        ...
        for page_number in range(page_length):
            ...
            if len(existing_links) >= len(links):
                print(f'All links are already available at page {page_number+1}')
                break
            ...
            print(f'Committed {len(links)-len(existing_links)}/{len(links)} new link! page {page_number+1}')
        return new_links

# if __name__ == "__main__":
#     updater = PropertyLinkUpdater()
#     updater.fetch_new_links(10)
